...
function renderAch() {
  ...
  const keptSections = (context.keptSectionKeys || []).map((key) => {
    ...
    return { key, title: section && section.t, art: SECTION_ART[key] || COURSE.fallback };
  });
...
function answer(g) {
  ...
  if (answeredCard && !session.sectionKeys.includes(answeredCard.s)) {
    session.sectionKeys.push(answeredCard.s);
  }
