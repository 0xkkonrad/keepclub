...
async function renderShelf(asOverlay, say) {
  ...
  // The phone's Back gesture closes this the way it closes the lightbox and
  // leaves a session — one entry, one press, one code path. app.js keeps that
  // stack and is handed the way out; nothing else can be pushed over this
  // panel, since everything under it is inert while it is up.
  let stop = false;
  function closeOverlay(fromHistory) {
    if (stop && !fromHistory) { stop = false; history.back(); return; }
    stop = false;
  ...
  if (asOverlay) {
    ...
    if (typeof globalThis.pushStop === 'function') {
      stop = true;
      globalThis.pushStop('shelf', () => closeOverlay(true));
    }
    // Wrapped, not passed: a listener called with the click event would take it
    // for "this came from a Back press" and leave the history entry standing.
 ...
 *
 * Only from a picker entry: as an overlay there is a course entry underneath
 * already, and stacking a second one would make Back from the new course land
 * on the old one. Read off the entry rather than passed in, because the
 * importer reloads from three screens deep and this is the same question there.
 */
function enterCourse(id) {
  localStorage.setItem(MUNIN.lastKey, id);
  const st = history.state;
  if (!st || st.munin === 'shelf') history.pushState({ munin: 'course' }, '');
  location.reload();
}
