...
export const CSS = `
  ...
  .imp-diags { margin: 14px 0 0; padding-left: 22px; display: grid; gap: 12px; }
  .imp-diags li { padding-left: 3px; font-size: .82rem; }
  .imp-diags code { display: block; color: var(--accent); font: 500 .72rem var(--mono);
    overflow-wrap: anywhere; }
  .imp-diags span, .imp-diags small { display: block; margin-top: 3px; }
  .imp-diags small { color: var(--muted); line-height: 1.45; }
  .imp-docs { color: var(--accent); text-align: center; font-size: .82rem;
    min-height: var(--tap); display: grid; place-items: center; }
  .imp-update { border-left: 4px solid var(--accent); padding-left: 12px;
...
function keepBook(r) {
  const landed = [];
  landed.push(`<li><b>${n(r.made.cards)}</b><span>${r.made.cards === 1 ? 'card' : 'cards'} ready to study</span></li>`);
  landed.push(`<li class="said"><b></b><span>filed under ${plural(r.made.sections, 'section')}${
    r.made.groups ? ` in ${plural(r.made.groups, 'group')}` : ''}</span></li>`);
  if (r.frontOnly) {
    landed.push(`<li class="said"><b></b><span>${plural(r.frontOnly, 'front-only card')
    } — self-grade without revealing an answer</span></li>`);
  }
  const media = [];
  if (r.media.images) media.push(plural(r.media.images, 'image'));
  if (r.media.audio) media.push(plural(r.media.audio, 'audio file'));
  if (r.media.video) media.push(plural(r.media.video, 'video'));
  if (media.length) {
    landed.push(`<li class="said"><b></b><span>with ${media.join(', ')}${
  ...
  return `<div class="imp-book">
      <h3>what landed</h3><ul>${landed.join('')}</ul>
      ${warnings ? `<h3>worth checking</h3><ul>${warnings}</ul>` : ''}
      <h3>what stays stable</h3><ul>
        <li class="said"><b></b><span>card IDs decide which review progress survives a future update</span></li>
        <li class="said"><b></b><span>course files contain no scheduling history; studying stays on this device</span></li>
      </ul>
    </div>`;
}
...
export function book(r) {
  if (r.type === 'keep') return keepBook(r);
  return `<div class="imp-book">
...
export function receiptHtml(r, existing) {
  if (r.type === 'keep') {
    ...
    const update = delta
      ? `<p class="imp-update">This is an update to the same course: ${plural(delta.unchanged, 'card')
      } ${delta.unchanged === 1 ? 'keeps its' : 'keep their'} progress, ${
    ...
    ? `<button type="button" class="go" data-keep="replace">${delta
      ? 'install the update, keeping progress' : 'replace it and start over'}</button>
